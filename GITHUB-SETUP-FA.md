...
ثانیه صبر می‌رود سراغ مدل بعدی (این بات فقط **متن** تولید می‌کند؛ تصویر
اصلاً در کار نیست).
...
| **مدارشکن** | مدلی که در همین اجرا سهمیه‌اش تمام شده، در بقیهٔ فراخوانی‌های همان اجرا هم رد می‌شود؛ پس سهمیهٔ بقیهٔ مدل‌ها بی‌دلیل نمی‌سوزد. |
...
اگر می‌خواهی مستقیم یک مدل خاص را مجبورش کنی، `GEMINI_MODEL` را بگذار — ولی `GEMINI_MODEL_FALLBACKS` را خالی نکن، وگرنه
...
- برای تست، `dry_run` را تیک بزن (چیزی به کانال نمی‌رود)
